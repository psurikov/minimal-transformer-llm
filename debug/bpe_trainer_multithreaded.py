...
class BpeTrainerMultiThreaded:
    ...
    def train(self, input_path: str, vocab_size: int, special_tokens: list[str]):
        ...
        regex_pattern = "|".join([regex.escape(special_token) for special_token in special_tokens]) + "|" + BpeTrainerMultiThreaded.pretoken_regex
        ...
        with open(input_path, "rb") as file:
            ...
            desired_chunk_size = min(max(file_size_bytes // process_count, BpeTrainerMultiThreaded.chunk_min_size), file_size_bytes)
    ...
    def _train_chunks(self, input_path: str, chunks: list[tuple[int, int]], regex_pattern: str, special_tokens: list[bytes], iterations: int) -> tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
        pretokens = BpeTrainerMultiThreaded._collect_pretokens(input_path, chunks, regex_pattern, special_tokens)
    ...
    def _collect_pretokens(input_path: str, chunks: list[tuple[int, int]], regex_pattern: str, special_tokens: list[bytes]) -> Counter:
        ...
        with ThreadPoolExecutor(max_workers=len(chunks)) as executor:
            futures = [
                executor.submit(BpeTrainerMultiThreaded._pretokenize, input_path, chunk, regex_pattern, special_tokens)
                for chunk in chunks
            ]
    ...
    def _pretokenize(input_path: str, chunk: tuple[int, int], regex_pattern: str, special_tokens: list[bytes]) -> Counter[str]:
        ...
        read_size = BpeTrainerMultiThreaded.buffer_size
        with open(input_path, "rb") as file:
            read_parts = BpeTrainerMultiThreaded._split(file, chunk[0], chunk[1], read_size, special_tokens)
            file.seek(chunk[0])
    ...
    def _split(file: BinaryIO, start: int, end: int, desired_size: int, special_tokens: list[bytes]) -> list[tuple[int, int]]:
        ...
        while chunk_start < end:
            chunk_end = BpeTrainerMultiThreaded._adjust_chunk_end(file, chunk_end, end, special_tokens)
    ...
    def _adjust_chunk_end(file: BinaryIO, chunk_end: int, end: int, special_tokens: list[bytes]) -> int:
        ...
        file.seek(chunk_end)
        while chunk_end < end:
            read_count = max(min(BpeTrainerMultiThreaded.special_token_search_size, end - chunk_end), 0)
