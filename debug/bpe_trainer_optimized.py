...
class BpeTrainerOptimized:
    ...
    def _train_chunks(self, input_path: str, chunks: list[tuple[int, int]], regex_pattern: str, special_tokens: list[bytes], iterations: int) -> tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
        ...
        for iteration in range(iterations):
            ...
            while i < count:
                ...
                write = self._merge2(pretoken_ids_list[i], pretoken_counts[i], max_pair, replace_id, pair_counts)
                if write <= 1:
                    count -= 1
            ...
            #print(f"iteration: {span_time} seconds")
    ...
    @staticmethod
    def _merge2(ids: list[int],  ids_count: int, pair: tuple[int, int], replace_id: int, pair_counts: dict[tuple[int, int], int]) -> int:
        sentinel = -1
        n = len(ids)
        if n <= 1:
            return 0
